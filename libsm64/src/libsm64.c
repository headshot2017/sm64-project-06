...
SM64_LIB_FN void sm64_mario_tick( int32_t marioId, const struct SM64MarioInputs *inputs, struct SM64MarioState *outState, struct SM64MarioGeometryBuffers *outBuffers )
{
    ...
    outState->forwardVel = gMarioState->forwardVel;
    outState->faceAngle = (float)gMarioState->faceAngle[1] / 32768.0f * 3.14159f;
    outState->twirlYaw = (float)gMarioState->twirlYaw / 32768.0f * 3.14159f;
    ...
    outState->animID = gMarioState->marioObj->header.gfx.animInfo.animID;
    outState->animFrame = gMarioState->marioObj->header.gfx.animInfo.animFrame;
    outState->animTimer = gMarioState->marioObj->header.gfx.animInfo.animTimer;
    outState->animAccel = gMarioState->marioObj->header.gfx.animInfo.animAccel;
    outState->animStartFrame = gMarioState->marioObj->header.gfx.animInfo.curAnim->startFrame;
    outState->animLoopStart = gMarioState->marioObj->header.gfx.animInfo.curAnim->loopStart;
    outState->animLoopEnd = gMarioState->marioObj->header.gfx.animInfo.curAnim->loopEnd;
}
